...
async def test_find_or_create_session_creates_new_session_if_none_provided():
    ...
    mock_chat_repository.create_session = AsyncMock()
    ...
    mock_chat_repository.find_by_id.return_value = None # Should not find an existing session
    ...
    mock_chat_repository.find_by_id.assert_not_called()
...
async def test_find_or_create_session_finds_existing_session():
    ...
    mock_chat_repository.find_by_id.return_value = existing_session

    session_manager = SessionManager(mock_chat_repository)

    session = await session_manager.find_or_create_session(existing_session_id)

    mock_chat_repository.find_by_id.assert_called_once_with(existing_session_id)
